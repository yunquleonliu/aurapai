...
class LLMManager:
    ...
    async def _generate_llamacpp_stream(
        ...
        **kwargs: Any
        ...
        completion_url = f"{settings.LLAMACPP_SERVER_URL}/completion"
        ...
        # Override with direct prompt if llama.cpp server supports it and it's provided
        if prompt:
            payload["prompt"] = prompt
            # Ensure messages is not also sent if prompt is used
            if "messages" in payload:
                del payload["messages"]

        logger.debug(f"--- Sending payload to llama.cpp ---")
                    ...
                    if line.strip():
                        ...
                        if line_str.startswith('data: '):
                            try:
                                ...
                                logger.warning(f"Could not decode JSON from line: {line_str}")
                                continue
        except aiohttp.ClientConnectorError as e:
    ...
    async def generate_response(
        ...
        **kwargs: Any
        ...
        Aggregates the streaming response into a single LLMResponse object.
        """
        # Remove is_image_generation from kwargs and pass it explicitly
        is_image_generation = kwargs.pop('is_image_generation', False)
        full_response = []
